...
- **过程信息不抢正文的地方**：行动轨迹（时间线）**默认收起**（`max-h-[9rem]`，仍可滚动、仍跟最新），
  头部给「展开详细轨迹 / 收起轨迹」按钮，选择记在 localStorage。轨迹是"过程"，不是交付物。
- **自检结果分级显示**：通过时只留一句人话（诊断句 + 端点标识）；**不通过才**把技术细节
  （capabilities JSON、端点探测、每轮工具调用）摊开——那些信息是给排障用的，成功时摆出来只会挤占视线。
- **页面版式（2026-09-17 按用户反馈调整）**：
  - **行动轨迹是常驻侧栏**：宽屏（`xl`）下固定在页面右侧、宽 21rem、`sticky top-4`、
    高度 `calc(100vh-2rem)`、内部自己滚动 —— 用户在任何一个步骤都能瞄一眼最新进展，
    又不占正文的地方、也不打断操作；窄屏退回正文里的内联版本（默认收起、可展开）。
  - **多列网格**：创新点勾选区（`.plan-list`，≥1100px 两列）与逐条结论（`.findings-grid`，
    `xl` 两列）不再单列竖排 —— 单列在宽屏上空间利用率太低。
  - 主容器放宽到 `max-w-[1560px]`，给侧栏与网格留出宽度。
- **高亮要如实报告覆盖了多少**：引文里混着公式/符号时，PDF 文本层往往逐字匹配不上。
  `quote_rects` 是两级策略：① 字符串快路径（整段 → 8 词滑窗）；② **按词的几何序列匹配**
  —— 逐词找位置，再按读序（同行看 x、换行看行距）串起来，找不到的词（公式）跳过但不中断；
  覆盖率按**命中词下标集合**算（滑窗重叠，累加长度会把覆盖率算爆——踩过）。
  为什么必须有第二级：真论文里的连字符折行、非 ASCII 空格、两端对齐拉伸、双栏换列
  都会让"整段/整窗"字符串匹配失败，于是只亮出前半截（2026-09-17 用户实测）。
  几何匹配还带**防误报阈值**（覆盖率 ≥0.5 且最长连续命中 ≥4）——否则 "definitely not on this
  page" 这种常见词拼出来的假引文也会被串出一堆框。覆盖率 <1 时前端明说"只覆盖了约 N%，
  完整引文请看「原文文本」"，**不假装整段都高亮了**。
- **模型写的是 markdown，界面就必须渲染它**：追问回答与结论解释都交给
  `frontend/components/Markdown.tsx`（自写的极简渲染器：粗体/斜体/行内代码/链接/列表/引用/代码块/标题）。
  为什么自己写而不引第三方库：这些文本是**模型产出且会引用论文/代码里的不可信内容**，
  渲染器全程构造 React 节点、**从不使用 innerHTML**，所以原始 HTML/脚本在结构上不可能被执行；
  只需要那么几种写法，引一个完整解析器 + 插件链不划算。
- **一次提问只显示一条回答**：后端有并发保护（同一 run 同时只允许一个阶段，第二个请求 409），
  前端也不许把「服务端对话记录里的回答」和「事件流里残留的 assistant_text」各渲染一遍——
  `chatStreaming` 在 `chat_reply` 出现之后返回空（2026-09-16 用户实测"一句疑问回复两次"就是这个重复渲染）。
