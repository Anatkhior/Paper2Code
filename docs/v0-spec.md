...
- **点结论里的引用**：页面滚到对照阅读器，同时
  ...
  - 左栏切到「PDF 原版」时：跳到该页，并把引文前几个词拼进 iframe 的 `#search=`，
    借浏览器**内置查找**高亮同一段（Chromium/Firefox 支持；不支持时退回"只看页码"，
    并提示可切到「原文文本」看精确高亮）。内置 PDF 阅读器不允许外部脚本操作其 DOM，
    这是"在 PDF 原版里也高亮同一段"的现实上限。
  - 右栏（代码）：跳到引用行并高亮（原有行为）。
- 这些行为的断言：版式与顺序在 `m3_check`（静态 HTML 层面），
  阅读器的 PDF 查找高亮与自动滚动在 `m6_check`（打包产物层面）。
